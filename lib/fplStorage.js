...
const { UpstashRestDriver } = require('./upstashRestDriver');

/**
 * All FPL jobs share one logical store ("fpl-burster"), namespaced by key
 * prefix per concern. Backend is Upstash's REST API (UPSTASH_REDIS_URL +
 * UPSTASH_REDIS_TOKEN) via a custom driver — not ioredis/TCP. Local JSON
 * path still gets set (storage.js requires one) but on a GH Actions runner
 * it's just a throwaway mirror; it never persists between runs, which is
 * fine because init() reconciles from Redis every time.
 */
function openStore() {
  return getStorage('fpl-burster', path.join('data', 'fpl-burster.json'), {
    driver: new UpstashRestDriver(),
