...
Uses the same `storage.js` hybrid module as GoalBurster, backed here by a
custom `lib/upstashRestDriver.js` that talks to Upstash's **REST API**
(`UPSTASH_REDIS_URL` + `UPSTASH_REDIS_TOKEN`) over HTTPS rather than
ioredis/TCP. Local JSON is never persisted between runs since GitHub Actions
runners are ephemeral — `init()` reconciles from Redis fresh every run.
Note: REST has no live pub/sub, so `syncEnabled`/`subscribe()` is a no-op —
irrelevant here since each job is a single short-lived process, not a
long-running instance needing cross-process sync.
...
2. Set these as **repo secrets** (Settings → Secrets and variables → Actions):
   ...
   | Secret | Required for |
   |---|---|
   | `UPSTASH_REDIS_URL` | all jobs — REST endpoint, e.g. `https://xxxx.upstash.io` |
   | `UPSTASH_REDIS_TOKEN` | all jobs — REST token (not the ioredis/TCP connection string) |
